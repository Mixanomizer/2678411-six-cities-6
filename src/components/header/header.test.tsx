...
describe('Component: Header', () => {
  it('ссылку «Sign in» при отсутствии авторизации', () => {
  ...
  it('авторизованному показывает email и кнопку выхода', () => {
    ...
    const state = makeMockState({
      USER: { authorizationStatus: AuthorizationStatus.Auth, userData },
    });
    const { withStoreComponent } = withMockStore(<Header />, state);
    render(withStoreComponent);
  ...
  it('клик по «Sign out» инициирует выход', async () => {
    ...
    const { withStoreComponent, mockStore, mockAxiosAdapter } = withMockStore(
      <Header />,
      state,
    );
    ...
    expect(actionTypes.some((type) => type.startsWith('user/logout'))).toBe(
      true,
    );
  });
});
